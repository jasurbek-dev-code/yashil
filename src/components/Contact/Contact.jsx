...
import { usePostData } from '@/hooks/usePostData';
import toast from 'react-hot-toast';
import ReCAPTCHA from "react-google-recaptcha";  // ✅ qo‘shildi
import { useState } from "react";

export default function Contact() {
  const postMutation = usePostData('/about/contact');
  const { t } = useTranslation();
  const [recaptchaValue, setRecaptchaValue] = useState(null); // ✅ captcha state
  ...
  const onSubmit = async (data) => {
    if (!recaptchaValue) {
      toast.error("Iltimos, reCAPTCHA ni tasdiqlang!");
      return;
    }

    try {
      await postMutation.mutateAsync({ ...data, recaptcha: recaptchaValue });
      toast.success("Ma'lumot muvaffaqiyatli yuborildi!");
      reset();
        ...
        <div className="relative z-10 bg-white dark:bg-[#0f1a0f] backdrop-blur-md rounded-2xl shadow-xl max-w-[1200px] mx-auto overflow-hidden">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="px-5 pb-4 flex justify-end">
              <ReCAPTCHA
              //secret key:6LdflLUrAAAAAP5JlubnROLcBYR2OApyDT4z9C6-
                sitekey={"6LdflLUrAAAAAJa-yoLn0Ei3rLBfHQTV4MO5ff0Y"}
            ...
            <div className="flex justify-end px-5 pb-6">
              <button
                type="submit"
                disabled={!recaptchaValue} // ✅ captcha bo‘lmasa submit bo‘lmaydi
                className={`px-6 py-3 rounded-md transition duration-300 text-white ${recaptchaValue ? "bg-green-800 hover:bg-green-700" : "bg-gray-400 cursor-not-allowed"}`}
              >
                {t("contact_form.submit")}
              </button>
            </div>
